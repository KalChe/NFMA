...
def load_mnist_field(n_samples=1):
    transform = T.Compose([T.ToTensor()])
    dataset = torchvision.datasets.MNIST(root="./data", train=True, download=True, transform=transform)
    loader = torch.utils.data.DataLoader(dataset, batch_size=n_samples, shuffle=True, drop_last=True)
    imgs, labels = next(iter(loader))
    img = imgs[0,0].numpy()
    coords = np.stack(np.meshgrid(np.linspace(-1,1,28), np.linspace(-1,1,28)), -1).reshape(-1,2)
    values = img.reshape(-1,1)
    samples = np.concatenate([coords, values], axis=1)
    return torch.tensor(samples, dtype=torch.float32).to(device), labels[0].item()
...
def validate_affine_equivariance(operator, a=1.5, b=(0.3,-0.3)):
    samples, label = load_mnist_field()
    ...
    plt.subplot(1,3,1); plt.imshow(field_A); plt.title("f_S'(x)")
    plt.subplot(1,3,2); plt.imshow(field_B); plt.title("f_S((x-b)/a)")
    plt.subplot(1,3,3); plt.imshow(diff); plt.title(f"Diff MSE={mse:.4f}")
    plt.show()

    return mse, field_A, field_B, diff
...
def run_trials(n_trials=5):
    ...
    for t in range(n_trials):
        ...
        mse, fA, fB, fD = validate_affine_equivariance(operator)

        mses.append(mse)
        combined_A.append(fA)
        combined_B.append(fB)
        combined_D.append(fD)

    mean, sem = np.mean(mses), stats.sem(mses)
    ci95 = sem * stats.t.ppf(0.975, n_trials-1)
    print(f"\nFinal MSE over {n_trials} trials: {mean:.6f} ± {ci95:.6f} (95 percent CI)")

    avg_A = np.mean(np.stack(combined_A), axis=0)
    avg_B = np.mean(np.stack(combined_B), axis=0)
    avg_D = np.mean(np.stack(combined_D), axis=0)

    os.makedirs("figures", exist_ok=True)

    plt.figure(figsize=(12,4))
    plt.subplot(1,3,1); plt.imshow(avg_A); plt.title("Mean f_S'(x)")
    plt.subplot(1,3,2); plt.imshow(avg_B); plt.title("Mean f_S((x-b)/a)")
    plt.subplot(1,3,3); plt.imshow(avg_D); plt.title("Mean Abs Diff")
