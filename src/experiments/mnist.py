...
from ..utils import (
    set_seed, load_mnist_field, train_operator,
...
def run_mnist_experiment(
    ...
    # run mnist neural field experiment; returns dictionary with mse stats and fields
    ...
    for t in range(n_trials):
        ...
        train_operator(operator, load_mnist_field, num_iterations=num_iterations, verbose=False)

        mse, field_A, field_B = validate_affine_equivariance(operator, load_mnist_field)
...
def create_mnist_figure(results: dict, save: bool = True, verbose: bool = True):
    # create publication figure for mnist experiment
    ...
    fig.suptitle('MNIST Affine Equivariance Validation', fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    if save:
        save_figure(fig, 'mnist_affine_equivariance')
